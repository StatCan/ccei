 ...
 */
function ccei_install_tasks() {
  return [
    'ccei_install_extensions' => [
      'display_name' => t('Install extensions'),
      'display' => TRUE,
      'type' => 'batch',
    ],
    'ccei_import_language_config' => [
 ...
 */
function ccei_install_tasks_alter(array &$tasks, array $install_state) {
  ...
  $task = $tasks['ccei_import_language_config'];
  unset($tasks['ccei_import_language_config']);
  $tasks = array_merge($tasks, ['ccei_import_language_config' => $task]);
}
 ...
 */
function ccei_install_extensions(array &$install_state) {
  $batch = [];
  $modules = [
    'wxt_ext',
  ];
  foreach ($modules as $module) {
    $batch['operations'][] = ['ccei_install_module', (array) $module];
  }
  return $batch;
}
 ...
 */
function ccei_install_module($module) {
 ...
 *   The current install state.
 */
function ccei_import_language_config(array &$install_state) {
  ...
  $files = file_scan_directory(drupal_get_path('profile', 'ccei'), '/.*\.info\.yml/');
